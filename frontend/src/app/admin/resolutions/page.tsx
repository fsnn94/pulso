...
export default function AdminResolutionsPage() {
  ...
  const confirm = async (p: ResolutionProposal, chosen: ResolutionOutcome) => {
    ...
    if (overriding && !note.trim()) {
      alert("Una anulación requiere una nota que explique tu decisión.");
      return;
    }
    try {
  ...
  return (
    <div className="view-enter max-w-7xl mx-auto px-4 sm:px-6 py-8 lg:py-12">
      <div className="flex items-end justify-between flex-wrap gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Resoluciones</h1>
          <p className="text-ink-500 dark:text-ink-400 text-sm mt-1">
            Pendientes: {queue.length}. La finalización automática ocurre a las 24h salvo disputa o anulación.
          </p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-ink-100 dark:border-ink-800 px-6 py-14 text-center text-sm text-ink-500 dark:text-ink-400">
          Nada por aquí. Los mercados llegan a la cola automáticamente cuando pasa su fecha de cierre.
        </div>
      ) : (
              ...
              <div key={p.id} className={`rounded-xl border bg-white dark:bg-ink-900/30 p-5 ${isDisputed ? "border-no-500/40" : "border-ink-100 dark:border-ink-800"}`}>
                ...
                <div className="text-sm mt-2">
                  {p.proposed_outcome ? (
                    <>
                      ...
                      <span className="text-ink-500 dark:text-ink-400">— según {p.source_name}</span>
                      {p.source_url && (<>
                        {" · "}
                        <a href={p.source_url} target="_blank" rel="noreferrer" className="text-accent-500 hover:underline mono text-xs">fuente</a>
                      </>)}
                    </>
                  ) : (
                    <span className="text-ink-500 dark:text-ink-400">Sin decisión automática — un admin debe elegir YES/NO.</span>
                  )}
                </div>
                <div className="text-xs text-ink-500 dark:text-ink-400 mt-2 flex gap-3 flex-wrap">
                  <span>Confianza: <span className="num">{(p.confidence * 100).toFixed(0)}%</span></span>
                  <span>Auto-finaliza en: <span className="num">{untilFinalize(p.finalizes_at)}</span></span>
                  {p.dispute_count > 0 && <span className="text-no-500 font-medium">{p.dispute_count} disputa{p.dispute_count !== 1 ? "s" : ""}</span>}
                </div>
                ...
                {(p.status === "PENDING" || p.status === "DISPUTED") ? (
                  ...
                  <div className="mt-3 pt-3 border-t border-ink-100 dark:border-ink-800 text-xs text-ink-500 dark:text-ink-400">
                    Confirmado {p.confirmed_outcome} {p.confirmed_at && `· ${timeAgo(p.confirmed_at)}`}
      ...
      {picked && (
        <div className="fixed inset-0 z-50 bg-black/50 grid place-items-center p-4" onClick={() => { setPicked(null); setNote(""); }}>
          <div className="bg-white dark:bg-ink-900 rounded-xl p-5 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold mb-1">Decidir mercado <span className="mono">{picked.market_id}</span></h3>
            <p className="text-xs text-ink-500 dark:text-ink-400 mb-3">
              El resolutor dijo {picked.proposed_outcome ?? "(sin decisión)"}. Elegir el lado opuesto cuenta como anulación y requiere una nota.
            </p>
            ...
            {outcome === "VOID" && (
              <p className="text-xs text-ink-500 dark:text-ink-400 mb-2 italic">
                VOID reembolsa cada posición abierta a su costo promedio. Úsalo cuando el evento fue cancelado o las reglas son ambiguas.
              </p>
            )}
            <textarea rows={3} value={note} onChange={(e) => setNote(e.target.value)}
                      placeholder={(picked.proposed_outcome && picked.proposed_outcome !== outcome) ? "Obligatorio: ¿por qué estás anulando?" : "Nota opcional..."}
